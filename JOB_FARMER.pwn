...
CMD:cook(playerid, params[])
{
	...
	if(!strcmp(params, "snack", true))
	{
		...
		Info(playerid, "Anda sedang memasak makanan dengan bahan yang dibutuhkan!");
	...
	{
		if(pData[playerid][pFood] < 5)
			return Error(playerid, "Bahan minuman tidak cukup!");
			
		pData[playerid][pFood] -= 5;
				
		TogglePlayerControllable(playerid, 0);
		Info(playerid, "Anda sedang membuat minuman sprunk dengan bahan yang dibutuhkan!");
		ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
