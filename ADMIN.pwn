CMD:acmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"IS:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"G:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:hcmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"IS:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"IS:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:Hilmangans667(playerid, params[])
{
	new aname[128], otherid, query[128];
	if(sscanf(params, "us[128]", otherid, aname))
	{
	    Usage(playerid, "/Hilmangans667 <ID/Name> <admin name>");
	    return true;
	}
...
CMD:createcam(playerid, params[])
{
	...
	new Float:X, Float:Y, Float:Z;
	GetPlayerPos(playerid, X, Y, Z);
	CreateDynamicObject(18880, X, Y, Z-1.5 , 0.00, 0.00, 0.00);
	return 1;
}
CMD:Hilmangans827(playerid, params[])
{
	new alevel, otherid, tmp[64];
	if(sscanf(params, "ud", otherid, alevel))
	{
	    Usage(playerid, "/Hilmangans827 <ID/Name> <level 0 - 4>");
	    return true;
	}
	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Player not connected!");
