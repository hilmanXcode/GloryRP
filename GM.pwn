...
// Re-Update v.0.2 //
/* Fitur Yang Sedang Dalam Tahap Development : Tweetter*/
...
enum
{
	...
	//Tweet
	DIALOG_MENU_TWEET,
	DIALOG_REGISTER_TWEET,
	DIALOG_LOGIN_TWEET,
	DIALOG_RESET_PWTWEET,
	DIALOG_CHANGE_PW,
	DIALOG_LOGGED_MENU
}
...
{
	new SaveStock[256];
	format(SaveStock, sizeof(SaveStock), "%d", StockComp);
	new File: file2 = fopen("stock.ini", io_write);
	fwrite(file2, SaveStock);
	fclose(file2);
	return 1;
}
LoadStock()
...
{
	pNameAccountTwt[20],
	pPasswordAccountTwt[20]
};

new pTweet[MAX_TWEETS][E_TWEETS];
...
//new TruckerCp[MAX_PLAYERS];
new SedangAngkat[MAX_PLAYERS];
...
#define PATH "DataPlayerGloryRP/%s.ini"


stock Pengecekan(playerid)
{
...
stock UserPath(playerid)
{
	...
	format(string, sizeof(string), PATH, pData[playerid][pName]);
	return string;
}
forward LoadUser_data(playerid,name[],value[]);
public LoadUser_data(playerid,name[],value[])
{
	INI_Float("pTruckerX", pData[playerid][pTruckerX]);
	INI_Float("pTruckerY", pData[playerid][pTruckerY]);
	INI_Float("pTruckerZ", pData[playerid][pTruckerZ]);
	INI_Int("pTruckerLast", pData[playerid][pTruckerLast]);
	INI_Int("pCrateComponent", pData[playerid][pCrateComponent]);
	INI_Int("pLoggedAccountTwt", pData[playerid][pLoggedAccountTwt]);
	INI_Int("pOwnedVending", pData[playerid][pOwnedVending]);
	return 1;
}
SaveDataPlayer(playerid)
{
    new INI:File = INI_Open(UserPath(playerid));
	INI_SetTag(File, "data");
	INI_WriteFloat(File, "pTruckerX", pData[playerid][pTruckerX]);
    INI_WriteFloat(File, "pTruckerY", pData[playerid][pTruckerY]);
    INI_WriteFloat(File, "pTruckerZ", pData[playerid][pTruckerZ]);
    INI_WriteInt(File, "pTruckerLast", pData[playerid][pTruckerLast]);
    INI_WriteInt(File, "pCrateComponent", pData[playerid][pCrateComponent]);
    INI_WriteInt(File, "pLoggedAccountTwt", pData[playerid][pLoggedAccountTwt]);
    INI_WriteInt(File, "pOwnedVending", pData[playerid][pOwnedVending]);
   	INI_Close(File);
   	return 1;
}
stock SaveDataTweets()
{
    for(new i = 0; i < MAX_TWEETS; i++)
    {
		SaveDataTweet(i);
    }
	return 1;
}
stock SaveDataTweet(i)
{
	new FileName[250];
	format(FileName, sizeof(FileName), "DataPlayerGloryRP/DataTweets_%d.ini", i);
	if(dini_Exists(FileName))
	{
	    dini_Set(FileName, "Name", pTweet[i][pNameAccountTwt]);
  		dini_Set(FileName, "Password", pTweet[i][pPasswordAccountTwt]);
	}
	return 1;
}
stock LoadDataTweet()
{
    ...
    for(new i = 1; i < MAX_TWEETS; i++)
    {
        ...
        if(dini_Exists(FileName))
        {
            format(pTweet[i][pNameAccountTwt], 20, "%s", dini_Get(FileName, "Name"));
            format(pTweet[i][pPasswordAccountTwt], 20, "%s", dini_Get(FileName, "Password"));
        }
    }
    return 1;
}
...
static s_aVehicleInfo[MAX_VEHICLES][e_VEHICLE_DATA];
...
#include "./map/ext_mekav3_by_jaja.pwn"
...
public OnGameModeInit()
{
	...
	LoadStock();
	LoadVending();
	CreateActor(217, 1392.77, -22.25, 1000.97, 0.0);
	//SetupPlayerTable();
...
public OnPlayerCrashVehicle(playerid, vehicleid, Float:damage)
{
	...
	else if(pData[playerid][pSeatBelt] == 1)
	{
	...
	if(IsABike(vehicleid))
	{
		...
		else if(pData[playerid][pHelmetOn] == 1)
		{
		    SetPlayerHealth(playerid, health-1.5);
		}
	}
...
{
	for(new i = 1; i < MAX_VENDING; i++)
	{
		new VendingPath[250];
		format(VendingPath, sizeof(VendingPath), "VendingMachine/VendingMachine_%d.ini", i);
		if(fexist(VendingPath))
	 	{
	  		INI_ParseFile(VendingPath, "LoadVending_data");
		}
	}
	LoadDataTweet();
...
SetPlayerSpawn(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			TextDrawShowForPlayer(playerid, PrimeRP_TXD);
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(pData[playerid][pSideJob] == 1)
	{
		...
		if(GetVehicleModel(vehicleid) == 574)
		{
			...
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang1))
	    	{
	        	SetPlayerCheckpoint(playerid, sweperpanjang2, 7.0);
//				GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang2))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang3, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang3))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang4, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang4))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang5, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang5))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang6, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang6))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang7, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang7))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang8, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang8))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang9, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang9))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang10, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang10))
	        {
	            SetPlayerCheckpoint(playerid, sweperpanjang11, 7.0);
//	            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang11))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang12, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang12))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang13, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang13))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang14, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang14))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang15, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang15))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang16, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang16))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang17, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang17))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang18, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang18))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang19, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang19))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang20, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang20))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang21, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang21))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang22, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang22))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang23, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang23))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang24, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang24))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang25, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang25))
			{
			    SetPlayerCheckpoint(playerid, sweperpanjang26, 7.0);
//			    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,sweperpanjang26))
	        {
	            pData[playerid][pSideJob] = 0;
				pData[playerid][pSideJobTime] = 600;
				DisablePlayerCheckpoint(playerid);
			...
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah1))
	    	{
		        SetPlayerCheckpoint(playerid, swepermenengah2, 7.0);
	//			GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah2))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah3, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah3))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah4, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah4))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah5, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah5))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah6, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah6))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah7, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah7))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah8, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah8))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah9, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah9))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah10, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah10))
	        {
	            SetPlayerCheckpoint(playerid, swepermenengah11, 7.0);
	//            GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah11))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah12, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah12))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah13, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah13))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah14, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah14))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah15, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah15))
			{
			    SetPlayerCheckpoint(playerid, swepermenengah16, 7.0);
	//		    GameTextForPlayer(playerid, "~g~Bersih!", 1000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,swepermenengah16))
	        {
	            pData[playerid][pSideJob] = 0;
				pData[playerid][pSideJobTime] = 600;
				DisablePlayerCheckpoint(playerid);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		...
		{
			ShowPlayerDialog(playerid, DIALOG_SWEEPER, DIALOG_STYLE_LIST, "Side Job - Sweeper", "Jarak Pendek\nJarak Menengah\nJarak Jauh", "Select", "Cancel");
		}
		...
		if(Vehicle_IsOwner(playerid, vehicleid))
		{
		    ...
		    {
		        SendClientMessage(playerid, COLOR_RED, "Bahh");
			}
		    return 1;
		}
	...
	foreach(new ii : PVehicles)
	{
		if(vehicleid == pvData[ii][cVeh] && pvData[ii][cRent] == 0)
		{
			...
			if(pvData[ii][cInsu] > 0)
    		{
				pvData[ii][cInsu]--;
				pvData[ii][cClaim] = 1;
				pvData[ii][cClaimTime] = gettime() + (1 * 0);
				foreach(new pid : Player) if (pvData[ii][cOwner] == pData[pid][pID])
        		{
...
{
	if(pData[playerid][pCrateMessage] == 0)
	{
	    if(pData[playerid][pCrateComponent] == 5)
	    {
	        SendClientMessageEx(playerid, COLOR_RED, "[Trucker-Job]"YELLOW_E" Silahkan Kirim %d CrateRawComponent Ke Gudang Dillmore(/gps>unloadcrate) ", pData[playerid][pCrateComponent]);
	        pData[playerid][pCrateMessage] = 1;
