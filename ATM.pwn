	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			...
			cache_get_value_name_int(i, "atmStockMoney", AtmData[id][atmStockMoney]);
			cache_get_value_name_int(i, "atmDurability", AtmData[id][atmDurability]);
...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE atms SET posx='%f', posy='%f', posz='%f', posrx='%f', posry='%f', posrz='%f', interior='%d', world='%d', atmStockMoney='%d', atmDurability='%d' WHERE id='%d'",
	AtmData[id][atmX],
	AtmData[id][atmY],
	AtmData[id][atmZ],
	AtmData[id][atmRX],
	...
	AtmData[id][atmInt],
	AtmData[id][atmWorld],
	AtmData[id][atmStockMoney],
	AtmData[id][atmDurability],
	id
	);
...
CMD:createatm(playerid, params[])
{
	...
	AtmData[id][atmStockMoney] = 100000;
	AtmData[id][atmDurability] = 50;
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO atms SET id='%d', posx='%f', posy='%f', posz='%f', posrx='%f', posry='%f', posrz='%f', interior='%d', world='%d', atmStockMoney='%d', atmDurability='%d'", id, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ], AtmData[id][atmRX], AtmData[id][atmRY], AtmData[id][atmRZ], GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), AtmData[id][atmStockMoney], AtmData[id][atmDurability]);
...
CMD:atm(playerid, params[])
{
	...
	{
		new tstr[128];
		new idatm = Iter_Free(ATMS);
		AtmData[idatm][atmDurability]--;
