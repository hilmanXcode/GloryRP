...
{
	if(pData[playerid][pFaction] != 1) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Kamu Tidak Memiliki Akses.");
	new otherid;
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/getdata [Name]");
	
	DisplayDatabase(playerid, otherid);
	return 1;
}
CMD:tracephone(playerid, params[])
{
	new number, Float:X, Float:Y, Float:Z;
//	if(pData[playerid][pUsingLaptop] < 1) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E" Kamu Harus Menggunakan Komputer");
	if(pData[playerid][pFaction] != 1) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Kamu Tidak Punya Akses.");
//  if(pData[playerid][pFactionRank] < 4 ) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Kamu Belum Mempelajari Ini(Minimal Rank 4).");
	if(sscanf(params, "i", number)) SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E" /tracephone [number]");
	else
	{
	    if(number == pData[playerid][pPhone]) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Jangan Lakukan Hal Bodoh.");
        if(pData[playerid][pJail] > 0) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Jangan Lakukan Hal Bodoh.");
      	new found = 0;
		foreach(new i : Player)
		{
            if(pData[i][IsLoggedIn] == false || !IsPlayerConnected(i)) return Error(playerid, "Player Sedang Offline!");
			if(pData[i][pPhone] == number)
			{
                if(pData[i][pJail] > 0) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Jangan Lakukan Hal Bodoh(Player Di Penjara).");
                if(GetPlayerInterior(i) != 0) return SendClientMessageEx(playerid, COLOR_RED, "INFO : "WHITE_E"Player Ini Sedang Berada Di Dalam Interior.");
                if(GetPlayerVirtualWorld(i) != 0) return SendClientMessageEx(playerid, COLOR_RED, "INFO : "WHITE_E"Player Ini Sedang Berada Di Dalam Interior.");
				found = 1;
  	            SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E" Nomor Terlacak.");
  	            GetPlayerPos(i, X, Y, Z);
  	            SetPlayerCheckpoint(playerid, X, Y, Z, 5.0);
	        }
		}
		if(found == 0) return SendClientMessageEx(playerid, COLOR_LBLUE, "INFO : "WHITE_E"Nomor Ini Tidak Terdaftar Di Database Negara.");
	}
	return 1;
}
