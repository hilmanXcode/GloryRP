...
new Text3D:VenLabel[MAX_VENDING];

new VendingInfo[MAX_VENDING][VendingMachine];

stock SaveVending()
{
	new idx = 1, File:file;
	new string[256];
	while(idx < MAX_VENDING)
	{
		format(string, 256, "%f|%f|%f|%d|%s|%d|%d|%d|%d|%d|\r\n",
		VendingInfo[idx][PosX],
		VendingInfo[idx][PosY],
		VendingInfo[idx][PosZ],
		VendingInfo[idx][VenMoney],
		VendingInfo[idx][VenOwner],
		VendingInfo[idx][VenStock],
		VendingInfo[idx][VenPrice],
		VendingInfo[idx][VenType],
		VendingInfo[idx][VenProdPrice],
		VendingInfo[idx][VenOwned]);
		if(idx == 1)
		{
		    file = fopen("vendingmachine.cfg", io_write);
		}
		else
		{
		    file = fopen("vendingmachine.cfg", io_append);
		}
		fwrite(file, string);
		fclose(file);
		idx++;
	}
	print("Vending Machine Berhasil Disimpan");
	return 1;
}
stock LoadVending()
{
	...
	new string[10000];
	new String[10000];
	new File:file = fopen("vendingmachine.cfg", io_read);
	if(file)
	{
	    new idx = 1;
	    while(idx < MAX_VENDING)
	    {
	        fread(file, string);
	        split(string, veninfo, '|');
	        VendingInfo[idx][PosX] = floatstr(veninfo[0]);
	        VendingInfo[idx][PosY] = floatstr(veninfo[1]);
	        VendingInfo[idx][PosZ] = floatstr(veninfo[2]);
	        VendingInfo[idx][VenMoney] = strval(veninfo[3]);
	        format(VendingInfo[idx][VenOwner], 50, "%s", veninfo[4]);
	        VendingInfo[idx][VenStock] = strval(veninfo[5]);
	        VendingInfo[idx][VenPrice] = strval(veninfo[6]);
	        VendingInfo[idx][VenType] = strval(veninfo[7]);
	        VendingInfo[idx][VenProdPrice] = strval(veninfo[8]);
	        VendingInfo[idx][VenOwned] = strval(veninfo[9]);
	        if(VendingInfo[idx][PosX])
	        {
	        	if(VendingInfo[idx][VenOwned] == 0)
	        	{
	        		if(VendingInfo[idx][VenType] == 0)
	        		{
			        	VendingInfo[idx][ObjectId] = CreateDynamicObject(1209, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
						format(String, sizeof(String), "Vending ID "RED_E"[%d]\n"YELLOW_E" Harga Vending ["GREEN_E"$%d"YELLOW_E"]\n /buy Untuk Membeli Vending Ini", idx, VendingInfo[idx][VenPrice]);
					...
					{
						VendingInfo[idx][ObjectId] = CreateDynamicObject(1776, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
						format(String, sizeof(String), "Vending ID "RED_E"[%d]\n"YELLOW_E" Harga Vending ["GREEN_E"$%d"YELLOW_E"]\n /buy Untuk Membeli Vending Ini", idx, VendingInfo[idx][VenPrice]);
						VenLabel[idx] = CreateDynamic3DTextLabel(String, COLOR_YELLOW, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 10);
					}
				}
				else if(VendingInfo[idx][VenOwned] == 1)
				{
					if(VendingInfo[idx][VenType] == 0)
	        		{
	        			format(VendingInfo[idx][VenOwner], 50, "%s", VendingInfo[idx][VenOwner]);
			        	VendingInfo[idx][ObjectId] = CreateDynamicObject(1209, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
	...
	print("Vending Machine Berhasil Dimuat");
	return 1;
}
...
/*VendingType(vendingid)
{
	if(VendingInfo[vendingid][VenType] == 1)
	{
		VendingInfo[vendingid][VenModel] = 1776;
	}
	if(VendingInfo[vendingid][VenType] == 2)
	{
		VendingInfo[vendingid][VenModel] = 1209;
	}
}*/
...
//------------[ Bisnis Command ]------------
//	Vending System Masih Dalam Tahap Pengembangan.
CMD:createvending(playerid, params[])
{
	new String[10000];
	if(pData[playerid][pAdmin] < 6) return PermissionError(playerid);
	if(isnull(params)) return Usage(playerid, "/createvending [Drink/Food]");
	if(!strcmp(params, "drink", true))
	{
		for(new idx=1; idx < MAX_VENDING; idx++)
		{
		    if(!VendingInfo[idx][PosX])
		    {
				new Float:X, Float:Y, Float:Z;
				GetPlayerPos(playerid, X, Y, Z);
				format(VendingInfo[idx][VenOwner], MAX_PLAYER_NAME, "-");
		  		VendingInfo[idx][PosX] = X;
				VendingInfo[idx][PosY] = Y;
				VendingInfo[idx][PosZ] = Z;
				VendingInfo[idx][VenStock] = 50;
				VendingInfo[idx][VenMoney] = 0;
				VendingInfo[idx][VenProdPrice] = 5;
				VendingInfo[idx][VenPrice] = 50000;
				VendingInfo[idx][VenOwned] = 0;
				VendingInfo[idx][VenType] = 0;
		  		VendingInfo[idx][ObjectId] = CreateDynamicObject(1209, X, Y, Z, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
				format(String, sizeof(String), "Vending ID "RED_E"[%d]\n"YELLOW_E" Harga Vending ["GREEN_E"$%d"YELLOW_E"]\n /buy Untuk Membeli Vending Ini", idx, VendingInfo[idx][VenPrice]);
				VenLabel[idx] = CreateDynamic3DTextLabel(String, COLOR_YELLOW, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 10);
				idx = MAX_VENDING;
				SaveVending();
	...
	if(!strcmp(params, "food", true))
	{
		for(new idx=1; idx < MAX_VENDING; idx++)
		{
		    if(!VendingInfo[idx][PosX])
		    {
				...
				VendingInfo[idx][VenPrice] = 50000;
				VendingInfo[idx][VenOwned] = 0;
				VendingInfo[idx][VenType] = 1;
		  		VendingInfo[idx][ObjectId] = CreateDynamicObject(1776, X, Y, Z, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
				format(String, sizeof(String), "Vending ID "RED_E"[%d]\n"YELLOW_E" Harga Vending ["GREEN_E"$%d"YELLOW_E"]\n /buy Untuk Membeli Vending Ini", idx, VendingInfo[idx][VenPrice]);
				VenLabel[idx] = CreateDynamic3DTextLabel(String, COLOR_WHITE, VendingInfo[idx][PosX], VendingInfo[idx][PosY], VendingInfo[idx][PosZ], 10);
				idx = MAX_VENDING;
				SaveVending();
			}
		}
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}Glory:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
