...
CMD:mycrate(playerid, params[])
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER && IsATruck(vehicleid))
	{
		new mycrate[250];
		format(mycrate, sizeof(mycrate), "Crate Yang Ada Di Truck Mu : %d", pData[playerid][pCrateComponent]);
		ShowPlayerDialog(playerid, 999, DIALOG_STYLE_MSGBOX, "MyCrate", mycrate, "CLOSE", "");
	}
	return 1;
}
